...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\Program Files\\Desktop\\下学期课程论文\\心理实验\\心里语言和病理\\实验5.1\\listen and see.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "exe" ---
    ...
    for thisTrial in trials:
        ...
        # --- Prepare to start Routine "exe" ---
        # create an object to store info about Routine exe
        exe = data.Routine(
            name='exe',
            components=[movie, polygon, key_resp_2, text_5, text_8, text_9, text_10],
        )
        exe.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        movie.setMovie(name)
        ...
        # store start times for exe
        exe.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        exe.tStart = globalClock.getTime(format='float')
        exe.status = STARTED
        thisExp.addData('exe.started', exe.tStart)
        exe.maxDuration = None
        # keep track of which components have finished
        exeComponents = exe.components
        for thisComponent in exe.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        if isinstance(trials, data.TrialHandler2) and thisTrial.thisN != trials.thisTrial.thisN:
            continueRoutine = False
        exe.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                exe.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in exe.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in exe.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for exe
        exe.tStop = globalClock.getTime(format='float')
        exe.tStopRefresh = tThisFlipGlobal
        thisExp.addData('exe.stopped', exe.tStop)
        ...
        # the Routine "exe" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
    ...
    for thisTrial_3 in trials_3:
        ...
        trial1 = data.Routine(
            name='trial1',
            components=[polygon_2, movie_2, key_resp_5, text_6],
        )
        trial1.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        movie_2.setMovie(name)
        ...
        while continueRoutine:
            ...
            if movie_2.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if movie_2.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > movie_2.tStartRefresh + 3.0-frameTolerance or movie_2.isFinished:
                    # keep track of stop time/frame for later
            ...
            if text_6.status == NOT_STARTED and tThisFlip >= 4.0-frameTolerance:
                # keep track of start time/frame for later
