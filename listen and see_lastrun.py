...
expName = 'listen and see'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\Program Files\\Desktop\\实验5.1\\listen and see_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text_5 = visual.TextStim(win=win, name='text_5',
        text='',
        font='Arial',
        pos=(-0.21, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-2.0);
    text_8 = visual.TextStim(win=win, name='text_8',
        text='',
        font='Arial',
        pos=(-0.07, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-3.0);
    ...
    while continueRoutine:
        ...
        if key_resp.status == STARTED and not waitOnFlip:
            theseKeys = key_resp.getKeys(keyList=["x"], ignoreKeys=["escape"], waitRelease=False)
    ...
    # create an object to store info about Routine exe_2
    exe_2 = data.Routine(
        name='exe_2',
        components=[text_11, key_resp_6],
    )
    exe_2.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    key_resp_6.rt = []
    _key_resp_6_allKeys = []
    # store start times for exe_2
    exe_2.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    exe_2.tStart = globalClock.getTime(format='float')
    exe_2.status = STARTED
    thisExp.addData('exe_2.started', exe_2.tStart)
    exe_2.maxDuration = None
    # keep track of which components have finished
    exe_2Components = exe_2.components
    for thisComponent in exe_2.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "exe_2" ---
    exe_2.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            exe_2.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in exe_2.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in exe_2.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for exe_2
    exe_2.tStop = globalClock.getTime(format='float')
    exe_2.tStopRefresh = tThisFlipGlobal
    thisExp.addData('exe_2.stopped', exe_2.tStop)
    # check responses
    ...
    trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('exe.xlsx'), 
        seed=None, 
    )
    ...
    for thisTrial in trials:
        ...
        # create an object to store info about Routine we
        we = data.Routine(
            name='we',
            components=[polygon, key_resp_2, text_5, text_8, text_9, text_10, movie],
        )
        we.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        ...
        movie.setMovie(name)
        # store start times for we
        we.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        we.tStart = globalClock.getTime(format='float')
        we.status = STARTED
        thisExp.addData('we.started', we.tStart)
        we.maxDuration = None
        # keep track of which components have finished
        weComponents = we.components
        for thisComponent in we.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        if isinstance(trials, data.TrialHandler2) and thisTrial.thisN != trials.thisTrial.thisN:
            continueRoutine = False
        we.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            if key_resp_2.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_2.getKeys(keyList=['d','f','j','k',], ignoreKeys=["escape"], waitRelease=False)
            ...
            if text_9.status == NOT_STARTED and tThisFlip >= 5-frameTolerance:
                # keep track of start time/frame for later
            ...
            if text_10.status == NOT_STARTED and tThisFlip >= 5.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if movie.status == NOT_STARTED and tThisFlip >= 1.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                we.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in we.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        for thisComponent in we.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for we
        we.tStop = globalClock.getTime(format='float')
        we.tStopRefresh = tThisFlipGlobal
        thisExp.addData('we.stopped', we.tStop)
        # check responses
    ...
    # create an object to store info about Routine enter
    enter = data.Routine(
        name='enter',
        components=[text_3],
    )
    enter.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # store start times for enter
    enter.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    enter.tStart = globalClock.getTime(format='float')
    enter.status = STARTED
    thisExp.addData('enter.started', enter.tStart)
    enter.maxDuration = None
    # keep track of which components have finished
    enterComponents = enter.components
    for thisComponent in enter.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "enter" ---
    enter.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine and routineTimer.getTime() < 5.0:
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            enter.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in enter.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    for thisComponent in enter.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for enter
    enter.tStop = globalClock.getTime(format='float')
    enter.tStopRefresh = tThisFlipGlobal
    thisExp.addData('enter.stopped', enter.tStop)
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if enter.maxDurationReached:
        routineTimer.addTime(-enter.maxDuration)
    elif enter.forceEnded:
        routineTimer.reset()
    else:
    ...
    for thisTrial_3 in trials_3:
        ...
        trial1 = data.Routine(
            name='trial1',
            components=[polygon_2, key_resp_5, text_6, text_12, text_13, text_14, sound_1],
        )
        ...
        sound_1.setSound(name, secs=4, hamming=True)
        ...
        while continueRoutine:
            ...
            if key_resp_5.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_5.getKeys(keyList=['d','f','j','k'], ignoreKeys=["escape"], waitRelease=False)
            ...
            if text_12.status == NOT_STARTED and tThisFlip >= 5.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if text_13.status == NOT_STARTED and tThisFlip >= 5.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if text_14.status == NOT_STARTED and tThisFlip >= 5.0-frameTolerance:
                # keep track of start time/frame for later
